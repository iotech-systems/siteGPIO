...
class lctech4chModbus(redisHook, modbusBoard, threading.Thread):
   ...
   def set_channel(self, chnl: int, val: bool):
      ...
      def on_rval_0(dsent: bytearray) -> bool:
         ...
         _msg: str = "SET_OK" if bval else "SET_ERROR"
         utils.printf(_msg, tcCOLORS.green, bold=True, with_ts=True)
         return bval
      # -- -- -- -- -- -- -- --
      chnl = (chnl - 1)
   ...
   def set_channel_state(_self, chnl: int, val: bool):
      ...
      try:
         ...
         def on_rval_0(dsent: bytearray) -> bool:
            ...
            _mm: str = "SET_OK" if bval else "SET_ERROR"
            utils.printf(_mm, col=tcCOLORS.green, with_ts=False)
            return bval
         # -- -- -- -- -- -- -- --
         chnl = (chnl - 1)
         if chnl not in range(0, 4):
